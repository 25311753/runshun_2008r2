...
#pragma resource "*.dfm"
TDicBargainForm *DicBargainForm;
//---------------------------------------------------------------------------
void DicBargain(int nAuth)
{
  TDicBargainForm *pForm;
	try
	{
  	        pForm=new TDicBargainForm(Application);
        }
...
__fastcall TDicBargainForm::TDicBargainForm(TComponent* Owner)
        : TForm(Owner)
{                      
  m_enWorkState=EN_IDLE;
}
//---------------------------------------------------------------------------              
void TDicBargainForm::Row2Editor()
{
  TListItem *pItem;
...
void TDicBargainForm::RefreshData()
{
  char strBuff[512];
  TListItem *pItem;
  ...
  try
  {
    ...
    dm1->Query1->SQL->Add("select * from DicBargain");
    ...
    while(!dm1->Query1->Eof)
    {
      ...
      pItem->Caption=dm1->Query1->FieldByName("bgname")->AsString;
      dm1->Query1->Next();
    }
  }
  catch(...)
...
void TDicBargainForm::ResetCtrl()
{
...
void __fastcall TDicBargainForm::FormKeyDown(TObject *Sender, WORD &Key,
      TShiftState Shift)
{
  if (Key==VK_RETURN)
...
void __fastcall TDicBargainForm::FormShow(TObject *Sender)
{
  edtName->Text="";
  RefreshData();
  m_enWorkState=EN_IDLE;
...
void __fastcall TDicBargainForm::ListView1Click(TObject *Sender)
{
        Row2Editor();
	ResetCtrl();        
}
...
void __fastcall TDicBargainForm::btnOK0Click(TObject *Sender)
{
  ...
  {
    case EN_ADDNEW:
      sprintf(strSQL,"insert into DicBargain values('%s')",strName);
      break;
    case EN_EDIT:
      {TListItem *pItem = ListView1->Selected;
      sprintf(strSQL,"update DicBargain set bgname='%s' where bgname='%s'",strName,pItem->Caption.c_str());
      }break;
    default:
  ...
  try
  {
    ...
    {
      char strAddSQL[256];
      sprintf(strAddSQL,"select * from DicBargain where bgname='%s'",strName);
...
void __fastcall TDicBargainForm::btnCancel0Click(TObject *Sender)
{
  int nState;
...
void __fastcall TDicBargainForm::btnAddNewClick(TObject *Sender)
{
  edtName->Text="";
...
void __fastcall TDicBargainForm::btnEditClick(TObject *Sender)
{
...
void __fastcall TDicBargainForm::btnDeleteClick(TObject *Sender)
{
  ...
  sprintf(strSQL,"delete from DicBargain where bgname='%s'",edtName->Text.c_str());
  if(!dm1->OpenDatabase())  return;
...
void __fastcall TDicBargainForm::btnRefreshClick(TObject *Sender)
{
  RefreshData();
  ResetCtrl();          
}
...
void __fastcall TDicBargainForm::btnExitClick(TObject *Sender)
{
Close();        
}
//---------------------------------------------------------------------------
