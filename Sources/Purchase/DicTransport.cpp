...
#pragma resource "*.dfm"
TDicTransportForm *DicTransportForm;
//---------------------------------------------------------------------------
void DicTransport(int nAuth)
{
  TDicTransportForm *pForm;
	try
	{
  	        pForm=new TDicTransportForm(Application);
        }
...
__fastcall TDicTransportForm::TDicTransportForm(TComponent* Owner)
        : TForm(Owner)
{
  m_enWorkState=EN_IDLE;
}
//---------------------------------------------------------------------------
void __fastcall TDicTransportForm::FormKeyDown(TObject *Sender, WORD &Key,
      TShiftState Shift)
{
  if (Key==VK_RETURN)
...
//---------------------------------------------------------------------------
void __fastcall TDicTransportForm::FormShow(TObject *Sender)
{
  edtName->Text="";
  RefreshData();
  m_enWorkState=EN_IDLE;
...
//---------------------------------------------------------------------------
void __fastcall TDicTransportForm::ListView1Click(TObject *Sender)
{
        Row2Editor();
	ResetCtrl();        
}
//---------------------------------------------------------------------------
void __fastcall TDicTransportForm::btnOK0Click(TObject *Sender)
{
  ...
  {
    case EN_ADDNEW:
      sprintf(strSQL,"insert into DicTransport values('%s')",strName);
      break;
    case EN_EDIT:
      {
        TListItem *pItem = ListView1->Selected;
        sprintf(strSQL,"update DicTransport set tpname='%s' where tpname='%s'",strName,pItem->Caption.c_str());
      }
      break;
    default:
  ...
  try
  {
    ...
    {
      char strAddSQL[256];
      sprintf(strAddSQL,"select * from DicTransport where tpname='%s'",strName);
...
//---------------------------------------------------------------------------
void __fastcall TDicTransportForm::btnCancel0Click(TObject *Sender)
{
  int nState;
...
//---------------------------------------------------------------------------
void __fastcall TDicTransportForm::btnAddNewClick(TObject *Sender)
{
  edtName->Text="";
...
//---------------------------------------------------------------------------
void __fastcall TDicTransportForm::btnEditClick(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TDicTransportForm::btnDeleteClick(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TDicTransportForm::btnRefreshClick(TObject *Sender)
{
  RefreshData();
  ResetCtrl();             
}
//---------------------------------------------------------------------------
void __fastcall TDicTransportForm::btnExitClick(TObject *Sender)
{
  Close();            
}
//---------------------------------------------------------------------------

void TDicTransportForm::Row2Editor()
{
  TListItem *pItem;
...
void TDicTransportForm::RefreshData()
{
  char strBuff[512];
  TListItem *pItem;
  ...
  try
  {
    ...
    dm1->Query1->SQL->Add("select * from DicTransport");
    ...
    {
    	pItem=ListView1->Items->Add();
      pItem->Caption=dm1->Query1->FieldByName("tpname")->AsString;
      dm1->Query1->Next();
    }
  }
  catch(...)
...
void TDicTransportForm::ResetCtrl()
{
