...
#pragma resource "*.dfm"
TDicBoatNoForm *DicBoatNoForm;
//---------------------------------------------------------------------------
void DicBoatNo(int nAuth)
{
  TDicBoatNoForm *pForm;
	try
	{
  	        pForm=new TDicBoatNoForm(Application);
        }
...
__fastcall TDicBoatNoForm::TDicBoatNoForm(TComponent* Owner)
        : TForm(Owner)
{
  m_enWorkState=EN_IDLE;
}
//---------------------------------------------------------------------------
void __fastcall TDicBoatNoForm::btnRefreshClick(TObject *Sender)
{
  RefreshData();
  ResetCtrl();        
}
//---------------------------------------------------------------------------
void TDicBoatNoForm::Row2Editor()
{
  TListItem *pItem;
...
void TDicBoatNoForm::RefreshData()
{
  char strBuff[512];
  TListItem *pItem;
  ...
  try
  {
    ...
    dm1->Query1->SQL->Add("select * from DicBoatNo");
    ...
    while(!dm1->Query1->Eof)
    {
      ...
      pItem->Caption=dm1->Query1->FieldByName("bname")->AsString;
      dm1->Query1->Next();
    }
  }
  catch(...)
...
void __fastcall TDicBoatNoForm::btnAddNewClick(TObject *Sender)
{
  edtName->Text="";
...
//---------------------------------------------------------------------------
void __fastcall TDicBoatNoForm::btnEditClick(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TDicBoatNoForm::btnCancel0Click(TObject *Sender)
{
  int nState;
...
//---------------------------------------------------------------------------
void __fastcall TDicBoatNoForm::btnOK0Click(TObject *Sender)
{
  ...
  {
    case EN_ADDNEW:
      sprintf(strSQL,"insert into DicBoatNo values('%s')",strName);
      break;
    case EN_EDIT:
      {
        TListItem *pItem = ListView1->Selected;
        sprintf(strSQL,"update DicBoatNo set bname='%s' where bname='%s'",strName,pItem->Caption.c_str());
      }
      break;
    default:
  ...
  try
  {
    ...
    {
      char strAddSQL[256];
      sprintf(strAddSQL,"select * from DicBoatNo where bname='%s'",strName);
...
//---------------------------------------------------------------------------
void __fastcall TDicBoatNoForm::btnDeleteClick(TObject *Sender)
{
...
void __fastcall TDicBoatNoForm::btnExitClick(TObject *Sender)
{
  Close();        
}
//---------------------------------------------------------------------------
void __fastcall TDicBoatNoForm::FormKeyDown(TObject *Sender, WORD &Key,
      TShiftState Shift)
{
  if (Key==VK_RETURN)
...
//---------------------------------------------------------------------------
void __fastcall TDicBoatNoForm::FormShow(TObject *Sender)
{
  edtName->Text="";
  RefreshData();
  m_enWorkState=EN_IDLE;
...
//---------------------------------------------------------------------------
void __fastcall TDicBoatNoForm::ListView1Click(TObject *Sender)
{
        Row2Editor();
	ResetCtrl();
}
