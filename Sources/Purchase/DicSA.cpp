...
#pragma resource "*.dfm"
TDicShipAgent *DicShipAgent;
//---------------------------------------------------------------------------
void DicShipAgent1(int nAuth)
{
  TDicShipAgent *pForm;
	try
	{
  	        pForm=new TDicShipAgent(Application);
        }
...
__fastcall TDicShipAgent::TDicShipAgent(TComponent* Owner)
        : TForm(Owner)
{
  m_enWorkState=EN_IDLE;
}
//---------------------------------------------------------------------------
void __fastcall TDicShipAgent::FormKeyDown(TObject *Sender, WORD &Key,
      TShiftState Shift)
{
  if (Key==VK_RETURN)
...
//---------------------------------------------------------------------------
void __fastcall TDicShipAgent::FormShow(TObject *Sender)
{
  edtName->Text="";
  RefreshData();
  m_enWorkState=EN_IDLE;
...
//---------------------------------------------------------------------------
void __fastcall TDicShipAgent::ListView1Click(TObject *Sender)
{
        Row2Editor();
	ResetCtrl();        
}
//---------------------------------------------------------------------------
void __fastcall TDicShipAgent::btnOK0Click(TObject *Sender)
{
  ...
  {
    case EN_ADDNEW:
      sprintf(strSQL,"insert into DicShipAgent values('%s')",strName);
      break;
    case EN_EDIT:
      {
        TListItem *pItem = ListView1->Selected;
        sprintf(strSQL,"update DicShipAgent set saname='%s' where saname='%s'",strName,pItem->Caption.c_str());
      }
      break;
    default:
  ...
  try
  {
    ...
    {
      char strAddSQL[256];
      sprintf(strAddSQL,"select * from DicShipAgent where saname='%s'",strName);
...
//---------------------------------------------------------------------------
void __fastcall TDicShipAgent::btnCancel0Click(TObject *Sender)
{
  int nState;
...
//---------------------------------------------------------------------------
void __fastcall TDicShipAgent::btnAddNewClick(TObject *Sender)
{
  edtName->Text="";
...
//---------------------------------------------------------------------------
void __fastcall TDicShipAgent::btnEditClick(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TDicShipAgent::btnDeleteClick(TObject *Sender)
{
  ...
  sprintf(strSQL,"delete from DicShipAgent where saname='%s'",edtName->Text.c_str());
  if(!dm1->OpenDatabase())  return;
...
//---------------------------------------------------------------------------
void __fastcall TDicShipAgent::btnRefreshClick(TObject *Sender)
{
  RefreshData();
  ResetCtrl();                
}
//---------------------------------------------------------------------------
void __fastcall TDicShipAgent::btnExitClick(TObject *Sender)
{
        Close();        
}
//---------------------------------------------------------------------------

void TDicShipAgent::Row2Editor()
{
  TListItem *pItem;
...
void TDicShipAgent::RefreshData()
{
  char strBuff[512];
  TListItem *pItem;
  ...
  try
  {
    ...
    dm1->Query1->SQL->Add("select * from DicShipAgent");
    ...
    {
    	pItem=ListView1->Items->Add();
      pItem->Caption=dm1->Query1->FieldByName("saname")->AsString;
      dm1->Query1->Next();
    }
  }
  catch(...)
