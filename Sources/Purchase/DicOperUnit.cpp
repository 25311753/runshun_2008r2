...
#pragma resource "*.dfm"
TDicOperUnitForm *DicOperUnitForm;
//---------------------------------------------------------------------------
__fastcall TDicOperUnitForm::TDicOperUnitForm(TComponent* Owner)
        : TForm(Owner)
{
...
void DicOperUnit(int nAuth)
{
  ...
  TDicOperUnitForm *pForm;
	try
	{
  	pForm=new TDicOperUnitForm(Application);
  }
...
void TDicOperUnitForm::Row2Editor()
{
  TListItem *pItem;
...
void TDicOperUnitForm::ResetCtrl()
{
...
void __fastcall TDicOperUnitForm::ListView1Click(TObject *Sender)
{
...
void __fastcall TDicOperUnitForm::btnOK0Click(TObject *Sender)
{
char strAddress[2000],strName[1000],strTel[80],*ptr,strTemp[80];
  int nState;
  ...
  {
    case EN_ADDNEW:
      sprintf(strSQL,"insert into dicoperunit(name,address,tel) values('%s','%s','%s')",strName,strAddress,strTel);
      break;
    case EN_EDIT:
      sprintf(strSQL,"update dicoperunit set address='%s',tel='%s' where name='%s'",strAddress,strTel,strName);
      break;
    default:
  ...
  try
  {
    ...
    {
      char strAddSQL[256];
      sprintf(strAddSQL,"select * from dicoperunit where name='%s'",strName);
...
void __fastcall TDicOperUnitForm::btnCancel0Click(TObject *Sender)
{
int nState;
...
void __fastcall TDicOperUnitForm::btnAddNewClick(TObject *Sender)
{
 edtAddress->Text="";
  edtTel->Text="";
  edtName->Text="";
...
void __fastcall TDicOperUnitForm::btnEditClick(TObject *Sender)
{
...
void __fastcall TDicOperUnitForm::btnDeleteClick(TObject *Sender)
{
 char strMsg[256],strSQL[512];
  ...
  sprintf(strSQL,"delete from dicoperunit where name='%s'",edtName->Text.c_str());
  if(!dm1->OpenDatabase())  return;
...
void __fastcall TDicOperUnitForm::btnRefreshClick(TObject *Sender)
{
...
void __fastcall TDicOperUnitForm::btnExitClick(TObject *Sender)
{
...
void TDicOperUnitForm::RefreshData()
{
  char strBuff[512];
  TListItem *pItem;
  ...
  try
  {
    ...
    dm1->Query1->SQL->Add("select * from dicoperunit");
...
void __fastcall TDicOperUnitForm::FormKeyDown(TObject *Sender, WORD &Key,
      TShiftState Shift)
{
...
void __fastcall TDicOperUnitForm::FormShow(TObject *Sender)
{
