...
#pragma resource "*.dfm"
TDicPackForm *DicPackForm;
//---------------------------------------------------------------------------
void DicPack(int nAuth)
{
  TDicPackForm *pForm;
	try
	{
  	        pForm=new TDicPackForm(Application);
        }
...
__fastcall TDicPackForm::TDicPackForm(TComponent* Owner)
        : TForm(Owner)
{
  m_enWorkState=EN_IDLE;
}
//---------------------------------------------------------------------------
void __fastcall TDicPackForm::FormKeyDown(TObject *Sender, WORD &Key,
      TShiftState Shift)
{
  if (Key==VK_RETURN)
...
//---------------------------------------------------------------------------
void __fastcall TDicPackForm::ListView1Click(TObject *Sender)
{
        Row2Editor();
	ResetCtrl();        
}
//---------------------------------------------------------------------------
void __fastcall TDicPackForm::btnOK0Click(TObject *Sender)
{
  ...
  {
    case EN_ADDNEW:
      sprintf(strSQL,"insert into DicPack values('%s')",strName);
      break;
    case EN_EDIT:
      {
        TListItem *pItem = ListView1->Selected;
        sprintf(strSQL,"update DicPack set pkname='%s' where pkname='%s'",strName,pItem->Caption.c_str());
      }
      break;
    default:
  ...
  try
  {
    ...
    {
      char strAddSQL[256];
      sprintf(strAddSQL,"select * from DicPack where pkname='%s'",strName);
...
//---------------------------------------------------------------------------
void __fastcall TDicPackForm::btnCancel0Click(TObject *Sender)
{
  int nState;
...
//---------------------------------------------------------------------------
void __fastcall TDicPackForm::btnAddNewClick(TObject *Sender)
{
  edtName->Text="";
...
//---------------------------------------------------------------------------
void __fastcall TDicPackForm::btnEditClick(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TDicPackForm::btnDeleteClick(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TDicPackForm::btnRefreshClick(TObject *Sender)
{
  RefreshData();
  ResetCtrl();                
}
//---------------------------------------------------------------------------
void __fastcall TDicPackForm::btnExitClick(TObject *Sender)
{
        Close();        
}
//---------------------------------------------------------------------------

void TDicPackForm::Row2Editor()
{
  TListItem *pItem;
...
void TDicPackForm::RefreshData()
{
  ...
  try
  {
    ...
    dm1->Query1->SQL->Add("select * from DicPack");
    ...
    {
    	pItem=ListView1->Items->Add();
      pItem->Caption=dm1->Query1->FieldByName("pkname")->AsString;
      dm1->Query1->Next();
    }
  }
  catch(...)
