...
#pragma resource "*.dfm"
TDicStatus1Form *DicStatus1Form;
//---------------------------------------------------------------------------
void DicStatus1(int nAuth)
{
  TDicStatus1Form *pForm;
	try
	{
  	        pForm=new TDicStatus1Form(Application);
        }
...
__fastcall TDicStatus1Form::TDicStatus1Form(TComponent* Owner)
        : TForm(Owner)
{
 m_enWorkState=EN_IDLE;
}
//---------------------------------------------------------------------------
void TDicStatus1Form::Row2Editor()
{
  TListItem *pItem;
...
void TDicStatus1Form::RefreshData()
{
  char strBuff[512];
  TListItem *pItem;
  ...
  try
  {
    ...
    dm1->Query1->SQL->Add("select * from DicStatus");
    ...
    while(!dm1->Query1->Eof)
    {
      ...
      pItem->Caption=dm1->Query1->FieldByName("sname")->AsString;
      dm1->Query1->Next();
    }
  }
  catch(...)
...
void TDicStatus1Form::ResetCtrl()
{
...
void __fastcall TDicStatus1Form::FormShow(TObject *Sender)
{
  edtName->Text="";
  RefreshData();
  m_enWorkState=EN_IDLE;
...
void __fastcall TDicStatus1Form::FormKeyDown(TObject *Sender, WORD &Key,
      TShiftState Shift)
{
  if (Key==VK_RETURN)
...
void __fastcall TDicStatus1Form::ListView1Click(TObject *Sender)
{
        Row2Editor();
	ResetCtrl();        
}
...
void __fastcall TDicStatus1Form::btnAddNewClick(TObject *Sender)
{
  edtName->Text="";
...
void __fastcall TDicStatus1Form::btnOK0Click(TObject *Sender)
{
  ...
  {
    case EN_ADDNEW:
      sprintf(strSQL,"insert into DicStatus values('%s')",strName);
      break;
    case EN_EDIT:
      {
        TListItem *pItem = ListView1->Selected;
        sprintf(strSQL,"update DicStatus set sname='%s' where sname='%s'",strName,pItem->Caption.c_str());
      }
      break;
    default:
  ...
  try
  {
    ...
    {
      char strAddSQL[256];
      sprintf(strAddSQL,"select * from DicStatus where sname='%s'",strName);
...
void __fastcall TDicStatus1Form::btnCancel0Click(TObject *Sender)
{
  int nState;
...
void __fastcall TDicStatus1Form::btnEditClick(TObject *Sender)
{
...
void __fastcall TDicStatus1Form::btnDeleteClick(TObject *Sender)
{
  ...
  sprintf(strSQL,"delete from DicStatus where sname='%s'",edtName->Text.c_str());
  if(!dm1->OpenDatabase())  return;
...
void __fastcall TDicStatus1Form::btnRefreshClick(TObject *Sender)
{
  RefreshData();
  ResetCtrl();           
}
...
void __fastcall TDicStatus1Form::btnExitClick(TObject *Sender)
{
        Close();        
}
//---------------------------------------------------------------------------
