...
#pragma resource "*.dfm"
TDicTradeForm *DicTradeForm;
//---------------------------------------------------------------------------
void DicTrade(int nAuth)
{
  TDicTradeForm *pForm;
	try
	{
  	        pForm=new TDicTradeForm(Application);
        }
...
__fastcall TDicTradeForm::TDicTradeForm(TComponent* Owner)
        : TForm(Owner)
{
  m_enWorkState=EN_IDLE;
}
//---------------------------------------------------------------------------
void __fastcall TDicTradeForm::FormKeyDown(TObject *Sender, WORD &Key,
      TShiftState Shift)
{
  if (Key==VK_RETURN)
...
//---------------------------------------------------------------------------
void __fastcall TDicTradeForm::FormShow(TObject *Sender)
{
  edtName->Text="";
  RefreshData();
  m_enWorkState=EN_IDLE;
...
//---------------------------------------------------------------------------
void __fastcall TDicTradeForm::ListView1Click(TObject *Sender)
{
        Row2Editor();
	ResetCtrl();        
}
//---------------------------------------------------------------------------
void __fastcall TDicTradeForm::btnOK0Click(TObject *Sender)
{
  ...
  {
    case EN_ADDNEW:
      sprintf(strSQL,"insert into DicTrade values('%s')",strName);
      break;
    case EN_EDIT:
      {
        TListItem *pItem = ListView1->Selected;
        sprintf(strSQL,"update DicTrade set trname='%s' where trname='%s'",strName,pItem->Caption.c_str());
      }
      break;
    default:
  ...
  try
  {
    ...
    {
      char strAddSQL[256];
      sprintf(strAddSQL,"select * from DicTrade where trname='%s'",strName);
...
//---------------------------------------------------------------------------
void __fastcall TDicTradeForm::btnCancel0Click(TObject *Sender)
{
  int nState;
...
//---------------------------------------------------------------------------
void __fastcall TDicTradeForm::btnAddNewClick(TObject *Sender)
{
  edtName->Text="";
...
//---------------------------------------------------------------------------
void __fastcall TDicTradeForm::btnEditClick(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TDicTradeForm::btnDeleteClick(TObject *Sender)
{
  ...
  sprintf(strSQL,"delete from DicTrade where trname='%s'",edtName->Text.c_str());
  if(!dm1->OpenDatabase())  return;
...
//---------------------------------------------------------------------------
void __fastcall TDicTradeForm::btnRefreshClick(TObject *Sender)
{
  RefreshData();
  ResetCtrl();         
}
...
void TDicTradeForm::RefreshData()
{
  char strBuff[512];
  TListItem *pItem;
  ...
  try
  {
    ...
    dm1->Query1->SQL->Add("select * from DicTrade");
    ...
    {
    	pItem=ListView1->Items->Add();
      pItem->Caption=dm1->Query1->FieldByName("trname")->AsString;
      dm1->Query1->Next();
    }
  }
  catch(...)
...
void __fastcall TDicTradeForm::btnExitClick(TObject *Sender)
{
Close();        
}
//---------------------------------------------------------------------------
