...
#ifndef __LAODY__FOUNDATION__CLASS__LFCTEMPL_H__
#define __LAODY__FOUNDATION__CLASS__LFCTEMPL_H__

//#ifndef __LFCPLEX_H__
//	#include "LfcPlex.h"   //    CList  ÉÆÇ»  ÀÞ¸³µÊ
//#endif

#ifdef _DEBUG
//static char _szAfxTempl[] = "LfcTempl.h";
#undef THIS_FILE
...
template<class TYPE>
inline void LFCAPI ConstructElements(TYPE* pElements, int nCount)
{
	ASSERT(nCount == 0 ||
...
//		::new((void*)pElements) TYPE;
		std::new((void*)pElements) TYPE;
}

template<class TYPE>
inline void LFCAPI DestructElements(TYPE* pElements, int nCount)
{
	ASSERT(nCount == 0 ||
		AfxIsValidAddress(pElements, nCount * sizeof(TYPE)));
...
template<class TYPE>
inline void LFCAPI CopyElements(TYPE* pDest, const TYPE* pSrc, int nCount)
{
	ASSERT(nCount == 0 ||
...
BOOL LFCAPI CompareElements(const TYPE* pElement1, const ARG_TYPE* pElement2)
{
...
inline UINT LFCAPI HashKey(ARG_KEY key)
{
...
#if _MSC_VER >= 1100
template<> void LFCAPI ConstructElements<CString> (CString* pElements, int nCount);
template<> void LFCAPI DestructElements<CString> (CString* pElements, int nCount);
template<> void LFCAPI CopyElements<CString> (CString* pDest, const CString* pSrc, int nCount);

template<> UINT LFCAPI HashKey<LPCSTR> (LPCSTR key);
#else // _MSC_VER >= 1100
void LFCAPI ConstructElements(CString* pElements, int nCount);
void LFCAPI DestructElements(CString* pElements, int nCount);
void LFCAPI CopyElements(CString* pDest, const CString* pSrc, int nCount);
UINT LFCAPI HashKey(LPCSTR key);
#endif // _MSC_VER >= 1100
...
#if _MSC_VER >= 1100
template<> UINT LFCAPI HashKey<const struct tagVARIANT&> (const struct tagVARIANT& var);
#else // _MSC_VER >= 1100
UINT LFCAPI HashKey(const struct tagVARIANT& var);
#endif // _MSC_VER >= 1100
...
#endif //__LAODY__FOUNDATION__CLASS__LFCTEMPL_H__
