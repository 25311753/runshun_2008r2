...
#include "BaseCode.h"
#include "DicDelivery.h"
#include "LdyInterface.h"
//---------------------------------------------------------------------------
#pragma package(smart_init)
#pragma resource "*.dfm"
TDicDeliveryForm *DicDeliveryForm;

// ==================================================================
//交货方式档输入接口，文件DicDelivery.*
void DicDelivery(int nAuth)
{
	if(nAuth==AUTH_NONE)
  {
  ...
  TDicDeliveryForm *pForm;
	try
	{
  	pForm=new TDicDeliveryForm(Application);
  }
...
__fastcall TDicDeliveryForm::TDicDeliveryForm(TComponent* Owner)
  : TForm(Owner)
{
...
void __fastcall TDicDeliveryForm::btnRefreshClick(TObject *Sender)
{
  RefreshData();
  ResetCtrl();
}
...
// --------------------------------------------------------------------------
void TDicDeliveryForm::Row2Editor()
{
  TListItem *pItem;
...
void TDicDeliveryForm::RefreshData()
{
  char strBuff[512];
  TListItem *pItem;
  ...
  try
  {
    ...
    dm1->Query1->SQL->Add("select * from DicDelivery");
...
void __fastcall TDicDeliveryForm::btnAddNewClick(TObject *Sender)
{
  edtID->Text="";
  edtName->Text="";
...
void __fastcall TDicDeliveryForm::btnEditClick(TObject *Sender)
{
...
void __fastcall TDicDeliveryForm::btnCancel0Click(TObject *Sender)
{
  int nState;
...
void __fastcall TDicDeliveryForm::btnOK0Click(TObject *Sender)
{
  ...
  {
    case EN_ADDNEW:
      sprintf(strSQL,"insert into DicDelivery values('%s','%s')",strID,strName);
      break;
    case EN_EDIT:
      sprintf(strSQL,"update DicDelivery set dd_name='%s' where dd_code='%s'",strName,strID);
      break;
    default:
  ...
  try
  {
    ...
    {
      char strAddSQL[256];
      sprintf(strAddSQL,"select * from DicDelivery where dd_code='%s'",strID);
...
void __fastcall TDicDeliveryForm::btnDeleteClick(TObject *Sender)
{
  ...
  sprintf(strSQL,"delete from DicDelivery where dd_code='%s'",edtID->Text.c_str());
  if(!dm1->OpenDatabase())  return;
...
void __fastcall TDicDeliveryForm::btnExitClick(TObject *Sender)
{
  Close();
}
//---------------------------------------------------------------------------
...
void __fastcall TDicDeliveryForm::FormKeyDown(TObject *Sender, WORD &Key,
      TShiftState Shift)
{
  if (Key==VK_RETURN)
...
void __fastcall TDicDeliveryForm::FormShow(TObject *Sender)
{
	edtID->Text="";
  edtName->Text="";
  RefreshData();
...
void __fastcall TDicDeliveryForm::ListView1Click(TObject *Sender)
{
  Row2Editor();
	ResetCtrl();
}
