...
#pragma resource "*.dfm"
TRawBookFactoryListForm *RawBookFactoryListForm;

//===============================================================================
//原料订购单列表查询接口，文件：RawBookFactoryList.*
void RawBookFactoryList(int nAuth)
{
	CALL_FORM(TRawBookFactoryListForm);
}
//======================================================================

//---------------------------------------------------------------------------
__fastcall TRawBookFactoryListForm::TRawBookFactoryListForm(TComponent* Owner)
	: TForm(Owner)
{
}
//---------------------------------------------------------------------------

void __fastcall TRawBookFactoryListForm::btnExitClick(TObject *Sender)
{
	Close();
}
//---------------------------------------------------------------------------
void __fastcall TRawBookFactoryListForm::FormShow(TObject *Sender)
{
	CString szSQL;
...
//---------------------------------------------------------------------------
void __fastcall TRawBookFactoryListForm::btnQueryClick(TObject *Sender)
{
...
//    " and bf_apcode=pd_code and pd_mncode=mn_code";
  ...
  if(!edtDate0->Text.IsEmpty())
  {
	...
	szSQL="select bf_code,bf_bookdate,bf_apcode,rh_name,mf_shortname,bf_mfcard,mn_code,mn_name,bf_count,bf_unit,bf_price,"
		"bf_dccode,ap_sdcode,ap_reqdepart,ap_shcode"
	...
	szSQL+=" group by bf_code,bf_bookdate,bf_apcode,rh_name,mf_shortname,bf_mfcard,mn_code,mn_name,bf_count,bf_unit,bf_price,"
		"bf_dccode,ap_sdcode,ap_reqdepart,ap_shcode";
...
//				fDef=dm1->Query1->FieldByName("am_def")->AsFloat;
    ...
    pItem->SubItems->Add(dm1->Query1->FieldByName("mn_name")->AsString);
		...
		pItem->SubItems->Add(FloatField2String(dm1->Query1->FieldByName("bf_count"),2));
                pItem->SubItems->Add(dm1->Query1->FieldByName("bf_unit")->AsString);
		if(strMNCode[1]=='M' && operatorcode!="admin" && dm1->Query1->FieldByName("ap_shcode")->AsString!=operatorcode && operatorcode!="108557" && operatorcode!="180079")
			bSeePrice=false;
		else	bSeePrice=true;
		if(!bSeePrice)
		...
		if(!bSeePrice)
			pItem->SubItems->Add(""); //只有采购经办自己才能看到原料总价
		else
...
//---------------------------------------------------------------------------
void __fastcall TRawBookFactoryListForm::btnToExcelClick(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TRawBookFactoryListForm::lstViewSelectItem(TObject *Sender,TListItem *Item, bool Selected)
{
	if(!Selected)	return;
...
//---------------------------------------------------------------------------
void __fastcall TRawBookFactoryListForm::lstViewClick(TObject *Sender)
{
	if(lstView->Selected==NULL)	ResetCtrl();
}

void TRawBookFactoryListForm::ResetCtrl()
{
...
void __fastcall TRawBookFactoryListForm::btnDetailClick(TObject *Sender)
{
	TListItem *pItem;
...
void __fastcall TRawBookFactoryListForm::lstViewDblClick(TObject *Sender)
{
	if(btnDetail->Enabled)	btnDetailClick(Sender);
}
//---------------------------------------------------------------------------

void __fastcall TRawBookFactoryListForm::lstFactoryExit(TObject *Sender)
{
	int nSel,i;
...
void __fastcall TRawBookFactoryListForm::btnPickFactClick(TObject *Sender)
{
...
void __fastcall TRawBookFactoryListForm::btnToAskClick(TObject *Sender)
{
	TListItem *pItem;
  char strAsk[80];
