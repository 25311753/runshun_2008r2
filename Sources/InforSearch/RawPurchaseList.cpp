...
#include "TConst.h"
#include "RawPurchaseList.h"
#include "LdyInterface.h"
//---------------------------------------------------------------------------
#pragma package(smart_init)
#pragma resource "*.dfm"
TRawPurchaseListForm *RawPurchaseListForm;

//===========================================================================
//原料请购单列表查询接口，文件：RawPurchaseList.*
void RawPurchaseList(int nAuth)
{
	TRawPurchaseListForm *pForm;
	pForm=new TRawPurchaseListForm(Application);
	assert(pForm!=NULL);
	pForm->ShowModal();
	delete pForm;
}
...
//---------------------------------------------------------------------------
__fastcall TRawPurchaseListForm::TRawPurchaseListForm(TComponent* Owner)
	: TForm(Owner)
{
}
//---------------------------------------------------------------------------
void __fastcall TRawPurchaseListForm::btnExitClick(TObject *Sender)
{
	Close();	
}

bool TRawPurchaseListForm::GetFilter(CString &szFilter)
{
	...
	{
		sprintf(strTemp," and substring(pd_mncode,1,2)='%cM'",g_theOperator.op_cpcode[0]);
		szFilter+=strTemp;
	}
	else
...
//---------------------------------------------------------------------------
void __fastcall TRawPurchaseListForm::btnQueryClick(TObject *Sender)
{
	TListItem *pItem;
  ...
  if(lstStockType->ItemIndex==1)	szFilter+=" and substring(ap_code,1,1)='Q'";
	...
	szSQL.Format("insert into %s"
		" select bf_code,bf_apcode,bf_bookdate,bf_deliverdays,mf_shortname,bf_count,sum(ar_quantity),sum(ar_checkquan),NULL,NULL"
		" from AskPurchase,PurchaseDetail,BookFactory,acceptraw,Manufactur"
		" where ap_code=substring(pd_code,1,7) and bf_apcode=pd_code"
		" and bf_mfcode=mf_code and bf_code*=ar_bfcode %s"
    ...
    if(strTemp[0]=='M')
			pItem->SubItems->Add("内购");
    else if(strTemp[0]=='N')
			pItem->SubItems->Add("外购");
    else if(strTemp[0]=='H')
			pItem->SubItems->Add("核销");
...
//---------------------------------------------------------------------------
void __fastcall TRawPurchaseListForm::FormShow(TObject *Sender)
{
	int nYear,nMonth,nDay;
...
void TRawPurchaseListForm::ResetReqDepart()
{
	CString szSQL;
	int nSel;
	bool bAll;
...
void TRawPurchaseListForm::ResetCtrl()
{
	btnDetail->Enabled = lstView->Selected!=NULL;
  btnToEnd->Enabled = lstView->Selected!=NULL;
}

void __fastcall TRawPurchaseListForm::lstViewSelectItem(TObject *Sender,TListItem *Item, bool Selected)
{
	if(!Selected)	return;
	ResetCtrl();
}
...
void __fastcall TRawPurchaseListForm::btnDetailClick(TObject *Sender)
{
	TListItem *pItem;
	char strID[80];
...
void __fastcall TRawPurchaseListForm::btnToExcelClick(TObject *Sender)
{
...
void __fastcall TRawPurchaseListForm::lstViewDblClick(TObject *Sender)
{
...
void __fastcall TRawPurchaseListForm::lstReqCorpClick(TObject *Sender)
{
	ResetReqDepart();	
}
//---------------------------------------------------------------------------

void __fastcall TRawPurchaseListForm::FormKeyDown(TObject *Sender,
...
void __fastcall TRawPurchaseListForm::FormResize(TObject *Sender)
{
...
void __fastcall TRawPurchaseListForm::btnToEndClick(TObject *Sender)
{
	TListItem *pItem;
	if((pItem=lstView->Selected)==NULL)	return;
	if(pItem->Caption.IsEmpty())	return;
...
void __fastcall TRawPurchaseListForm::btnMaterPickupClick(TObject *Sender)
{
